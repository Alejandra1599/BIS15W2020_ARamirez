...
```{r}
Ornith <- entrez_search(db="nuccore", term= "_Ornithorhynchus anatinus_[Organism]", retmax=40)
```

2. Use `entrez_fetch()` to return your database hits as a character vector.
```{r}
Ornith_seq<- entrez_fetch(db="nuccore", id= Ornith$ids, rettype = "fasta")
Ornith_seq
```

3. Now store your own pulled sequence data as a .fasta file using the `write()` function. Remember those line endings!
```{r}
write(Ornith_seq, "data/Ornith_fasta.fasta", sep = "\n")
```
